...
useful when you need to do many things to the same data set.  Pipes in R look like
...
`filter`, to keep rows where `Genotype` was equal to 'WT', and then through `select` to
keep the `SampleID` and `Sex` and `Genotype` columns. When the data frame is being passed to the
...
--->
    ...
    ##  male, and retain only the columns `SampleID` and `Genotype.`
...
for which their Weight was not recorded. We can remove the missing values for weight before we
